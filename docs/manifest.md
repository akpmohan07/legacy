...
| **`features/`** | **What each detector feature does**, one file each: exact rules, invariants, code map, the tests that guard them, how to verify by hand, known limits. The "what," precise enough to change safely. See `features/README.md`. | The reasoning behind a rule (that's `decisions/`); what a specific source can see (that's `sources/`) | `decisions/` by number; tests and code by name |
| **`sources/`** | **What each source covers**, one file per built source (scope in/out, identifier scheme, discovery, version and install-date availability, absence check, gaps), plus an index that also lists planned sources. See `sources/README.md`. | Feature rules (`features/`); how a scanner plugs in (`decisions/0001`) | `features/` and `decisions/` by name |
| **`detector/AGENTS.md`** (outside `docs/`) | **How to work in `detector/`**: commands, the never-run-without-asking rule, module map, the invariants to keep, checklists for adding a scanner, migration, or stored value. An entry point that links into `decisions/`, `features/`, and `sources/`. | Reasoning or behavior rules, which it points to instead | `decisions/`, `features/`, `sources/` |
...
`IDEATION.md` · `ARCHITECTURE.md` · `research-log.md` · `NORTH-STAR.md` · `related-products.md` · `manifest.md` · `CLAUDE.md` · `LESSONS.md` · `decisions/` · `features/` · `sources/` (and `detector/AGENTS.md`)
