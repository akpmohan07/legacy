...
- Detector stack: leaning Node/TS (same ecosystem as npm detection, keeps door open for the template renderer to share tooling) over Swift (macOS-only, ties to the same ecosystem `cli-tools` already occupies) — not yet confirmed. Affects which SQLite binding is used (`node:sqlite`/`better-sqlite3` vs. `SQLite3`/`libsqlite3`) but not the `WorkingStore` interface itself.
- `libsqlite3`-on-Linux path unverified — doesn't block v1 (macOS-only), but worth confirming before ever exercising the "swap the engine" option in earnest for a Linux port.
- `legacy.json`/`legacy.db` schema and the `WorkingStore` interface's exact method signatures — needs designing once detector work starts; the exported formats should stay stable/versioned since the future registry/aggregator (see above) depends on being able to fetch and parse everyone's file indefinitely.
