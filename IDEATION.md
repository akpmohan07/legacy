...
- **Local storage: SQLite as the detector's private working store, `legacy.json` as a generated export — not the same thing.** The "journey over time" reframing means the detector needs to accumulate scan history, diff snapshots into eras, and remember per-tool manual review decisions — query/diff-heavy work that a single flat file handles poorly (rewrite-the-whole-file-per-scan, no efficient "when did this first/last appear" lookup). SQLite stays zero-infrastructure (a local file, nothing to host, same category as `legacy.json` itself, just queryable) and is well-supported on both open detector-stack candidates (`better-sqlite3`/Bun's built-in on Node/TS; `SQLite.framework`, already built into macOS, on Swift) — doesn't bias that decision either way. Pipeline: scanners + usage readers write into SQLite (full private history, everything, never published) → on publish, a filtered snapshot (allowlist-visible items only) is exported → the manual review gate → written as `legacy.json` → hashed and anchored via OpenTimestamps. This doesn't reopen the "no database" deployment decision above — that was about a *server* database; this is local, embedded, on the user's own machine.
...
- `legacy.json` schema (and the SQLite table structure it's exported from) — needs designing once detector work starts; the exported format should stay stable/versioned since the future registry/aggregator (see above) depends on being able to fetch and parse everyone's file indefinitely.
